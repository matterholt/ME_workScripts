"""
script to create the next counter measure folder and
the copy files to new folder
"""
import shutil
import sys
import os
...
def main():
    """
    when script is executed the able to use a argv to supply script name
    of counter measueres either just number for the cm with no spaces or
    period, First to didgites are for version
    the rest are for the revision  (example XXXX, XX.XX.X)

    copy_content_folder --> contents inside folder will be copied
                    to working dir
    working_dir --> folder where current development of the project,
                         model saved and ran
    ...
    def check_argv_length(name_conv):
        """
        Step 2: convert argv to fit folder naming conventions
        convert the argv to a style of naming folder
        """
        list_name = name_conv.split(".")
        if len(list_name) >= 3:
            check_name_three = V + \
                list_name[0] + sep + R + list_name[1] + sep + list_name[2]
