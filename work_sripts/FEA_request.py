...
a Finite Element Analysis. The request explains what has change in the model,
what analysis and other data pertaining to the counter measure.

The problem that wish to solve is that the data that is collected is static,
 and is rarely looked at. Creating a database would help create summaries and
 see trends from previous models

This is a simple program asking user to type in data from FEA request
- check and test need to be implemented,
- main focus was to understand python classes and to get data from PPT 
    to json or csv
"""
import csv
import json
...
class CmData:
    ...
    def add_csv(self):
        """
        compile data to a csv file, Excel is the main program for data, 
        need to be universal and
        allow every one to access data
        - this file is going a sort of database that will have
         all Fea Request data in columns similar to how data is shared currently
        """
        # data_head = ["model name", "base model", "parts change",
        #  "description", "analysis", "weight(kg)","date" ]
        data_results = [self.name, self.base_model, self.part_changes,
...
def add_cm():
    ...
    FeaR_data = CmData(model_name, model_base, model_part_change,
                       model_description, model_analysis, model_weight, job_date)
